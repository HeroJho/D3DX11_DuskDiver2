...
HRESULT CComponent_Manager::Release_Component(_uint iLevelIndex, const _tchar * pPrototypeTag)
{
	for (auto& iter : m_pPrototypes[iLevelIndex])
	{

		if (!lstrcmp(iter.first, pPrototypeTag))
		{
			Safe_Release(iter.second);
			m_pPrototypes[iLevelIndex].erase(iter.first);
			return S_OK;
		}
	}
