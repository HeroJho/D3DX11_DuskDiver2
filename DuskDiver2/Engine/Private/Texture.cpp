...
HRESULT CTexture::Initialize_Prototype(const _tchar * pTextureFilePath, _uint iNumTextures, _bool bForce)
{
	...
	for (_uint i = 0; i < iNumTextures; ++i)
	{
		...
		if (false == lstrcmp(TEXT(".dds"), szExt))
		{
			...
			{
				hr = DirectX::CreateDDSTextureFromFileEx(m_pDevice, szFullPath, NULL, D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE,
					NULL,
					NULL,
					true,
					nullptr,
...
ID3D11ShaderResourceView * CTexture::Get_Texture(_uint iTextureNum)
{	
...
CTexture * CTexture::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, const _tchar * pTextureFilePath, _uint iNumTextures, _bool bForce)
{
	CTexture*			pInstance = new CTexture(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype(pTextureFilePath, iNumTextures, bForce)))
	{
