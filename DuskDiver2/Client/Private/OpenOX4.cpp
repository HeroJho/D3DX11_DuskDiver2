...
COpenOX4::COpenOX4(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

COpenOX4::COpenOX4(const COpenOX4 & rhs)
	: CGameObject(rhs)
{
}

HRESULT COpenOX4::Initialize_Prototype()
{
	return S_OK;
}

HRESULT COpenOX4::Initialize(void * pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(-15.f, 10.f, 87.3f, 1.f));

	return S_OK;
}

void COpenOX4::Tick(_float fTimeDelta)
{
	if (UM->Get_Trigger4())
	{
		if (!m_bTriggerCheck)
	...
	m_pTransformCom->Rotation(XMVectorSet( 0,1.f,0,0), -32.4f);

}

void COpenOX4::LateTick(_float fTimeDelta)
{
...
void COpenOX4::Render_Debug()
{
}

HRESULT COpenOX4::Ready_Components()
{
	/* For.Com_Transform */
...
COpenOX4 * COpenOX4::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	COpenOX4*		pInstance = new COpenOX4(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : COpenOX4"));
...
CGameObject * COpenOX4::Clone(void * pArg)
{
	COpenOX4*		pInstance = new COpenOX4(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : COpenOX4"));
		Safe_Release(pInstance);
	}
