...
void CWarning::Tick(_float fTimeDelta)
{
	...
	if (m_fTime > 1.f && m_fTime < 1.3f)
	{
		m_UiInfo.fSizeX += 2.f;
		m_UiInfo.fSizeX += 1.f;
	}
	//else if (m_fTime > 1.3f && m_fTime < 1.6f)
	//{
	//	m_UiInfo.fSizeX -= 5.f;
...
HRESULT CWarning::Ready_Components()
{

	/* For.Com_Texture */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Texture_Warning"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
