...
CMon_HP::CMon_HP(const CMon_HP & rhs)
	: CGameObject(rhs)
{
}

HRESULT CMon_HP::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMon_HP::Initialize(void * pArg)
{
...
void CMon_HP::Tick(_float fTimeDelta)
{
	if (!m_bBoss)
	{
		m_vMonsterPos = m_pMonster->Get_WorldPos();
...
void CMon_HP::LateTick(_float fTimeDelta)
{
...
HRESULT CMon_HP::Ready_Components()
{
	/* For.Com_Transform */
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Texture_Monster_HP"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;

	return S_OK;
}

HRESULT CMon_HP::Billboard()
{
...
CMon_HP * CMon_HP::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CMon_HP*		pInstance = new CMon_HP(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CMon_HP"));
...
CGameObject * CMon_HP::Clone(void * pArg)
{
	...
	{
		MSG_BOX(TEXT("Failed To Cloned : CMon_HP"));
		Safe_Release(pInstance);
	}
