...
CRealHolo_2::CRealHolo_2(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CRealHolo_2::CRealHolo_2(const CRealHolo_2 & rhs)
	: CGameObject(rhs)
{
}

HRESULT CRealHolo_2::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CRealHolo_2::Initialize(void * pArg)
{
	...
	XMStoreFloat3(&pDesc->vPos, m_pTransformCom->Get_State(CTransform::STATE_POSITION)
		...
		+ XMVector3Normalize(m_pTransformCom->Get_State(CTransform::STATE_LOOK)) * 1.09f);
	pDesc->vScale = { 6.f, 2.f, 1.f };
	...
	pDesc->vScale = { 6.f, 2.f, 4.f };
	m_pHolo_5_Blue = (CHolo_5_Blue*)GI->Clone_GameObject(TEXT("Holo_5_Blue"), pDesc);

	XMStoreFloat3(&pDesc->vPos, m_pTransformCom->Get_State(CTransform::STATE_POSITION)
		+ XMVector3Normalize(m_pTransformCom->Get_State(CTransform::STATE_RIGHT)) * 2.13f
		+ XMVector3Normalize(m_pTransformCom->Get_State(CTransform::STATE_UP)) * -0.1f
		+ XMVector3Normalize(m_pTransformCom->Get_State(CTransform::STATE_LOOK)) * -0.16f);
	pDesc->vScale = { 3.f, 2.f, 0.2f };
	m_pHolo_6 = (CHolo_6*)GI->Clone_GameObject(TEXT("Holo_6"), pDesc);

	XMStoreFloat3(&pDesc->vPos, m_pTransformCom->Get_State(CTransform::STATE_POSITION)
		+ XMVector3Normalize(m_pTransformCom->Get_State(CTransform::STATE_RIGHT)) * 1.76f
		+ XMVector3Normalize(m_pTransformCom->Get_State(CTransform::STATE_UP)) * -0.1f
		+ XMVector3Normalize(m_pTransformCom->Get_State(CTransform::STATE_LOOK)) * -0.91f);
	pDesc->vScale = { 2.f, 2.f, 1.f };
	m_pHolo_7 = (CHolo_7*)GI->Clone_GameObject(TEXT("Holo_7"), pDesc);

	XMStoreFloat3(&pDesc->vPos, m_pTransformCom->Get_State(CTransform::STATE_POSITION)
		+ XMVector3Normalize(m_pTransformCom->Get_State(CTransform::STATE_RIGHT)) * 2.25f
		+ XMVector3Normalize(m_pTransformCom->Get_State(CTransform::STATE_UP)) * -0.1f
		+ XMVector3Normalize(m_pTransformCom->Get_State(CTransform::STATE_LOOK)) * 0.71f);
	pDesc->vScale = { 4.f, 2.f, 1.5f };
...
void CRealHolo_2::Tick(_float fTimeDelta)
{
...
void CRealHolo_2::LateTick(_float fTimeDelta)
{
...
void CRealHolo_2::OnCollision(CCollider::OTHERTOMECOLDESC Desc)
{

}


HRESULT CRealHolo_2::Ready_Components()
{
	/* For.Com_Transform */
...
CRealHolo_2 * CRealHolo_2::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CRealHolo_2*		pInstance = new CRealHolo_2(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CRealHolo_2"));
...
CGameObject * CRealHolo_2::Clone(void * pArg)
{
	CRealHolo_2*		pInstance = new CRealHolo_2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CRealHolo_2"));
		Safe_Release(pInstance);
	}
