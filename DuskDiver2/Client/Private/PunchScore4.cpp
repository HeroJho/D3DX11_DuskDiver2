...
CPunchScore4::CPunchScore4(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CPunchScore4::CPunchScore4(const CPunchScore4 & rhs)
	: CGameObject(rhs)
{
}

HRESULT CPunchScore4::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CPunchScore4::Initialize(void * pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(138.5f, -13.f, 82.278f, 1.f));

	return S_OK;
}

void CPunchScore4::Tick(_float fTimeDelta)
{
...
void CPunchScore4::LateTick(_float fTimeDelta)
{
...
HRESULT CPunchScore4::Render()
{
	...
	if (m_bLight)
	{
		...
		{
			_int i = (_int)(TM->Get_FunchCuma()->Get_Score()) / 1000;
...
void CPunchScore4::Render_Debug()
{
...
HRESULT CPunchScore4::Ready_Components()
{
	/* For.Com_Transform */
...
CPunchScore4 * CPunchScore4::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CPunchScore4*		pInstance = new CPunchScore4(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CPunchScore4"));
...
CGameObject * CPunchScore4::Clone(void * pArg)
{
	CPunchScore4*		pInstance = new CPunchScore4(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CPunchScore4"));
		Safe_Release(pInstance);
	}
