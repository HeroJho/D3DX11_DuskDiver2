#include "stdafx.h"
#include "..\Public\NumStr.h"
#include "GameInstance.h"
#include "UI_Manager.h"


CNumStr::CNumStr(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CUI(pDevice, pContext)
{

}

CNumStr::CNumStr(const CNumStr & rhs)
	: CUI(rhs)
{
}

HRESULT CNumStr::Initialize_Prototype()
{
...
HRESULT CNumStr::Initialize(void * pArg)
{
...
void CNumStr::Tick(_float fTimeDelta)
{
...
void CNumStr::LateTick(_float fTimeDelta)
{
...
void CNumStr::Set_Number(_uint iNum)
{
	m_iNumber = iNum;
...
HRESULT CNumStr::Ready_Components()
{
...
CNumStr * CNumStr::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CNumStr*		pInstance = new CNumStr(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CNumStr"));
...
CGameObject * CNumStr::Clone(void * pArg)
{
	CNumStr*		pInstance = new CNumStr(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CNumStr"));
		Safe_Release(pInstance);
	}
