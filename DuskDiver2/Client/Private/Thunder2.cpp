...
void CThunder2::LateTick(_float fTimeDelta)
{
...
CThunder2 * CThunder2::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	...
	{
		MSG_BOX(TEXT("Failed To Created : CThunder2"));
...
{
	CThunder2*		pInstance = new CThunder2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CThunder2"));
		Safe_Release(pInstance);
	}
