#include "stdafx.h"
#include "..\Public\BeastMap3.h"
#include "GameInstance.h"
#include "Time_Manager.h"
#include "ToolManager.h"

CBeastMap3::CBeastMap3(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CEffect(pDevice, pContext)
{
}

CBeastMap3::CBeastMap3(const CBeastMap3 & rhs)
	: CEffect(rhs)
{
}

HRESULT CBeastMap3::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBeastMap3::Initialize(void * pArg)
{
	...
	m_pTransformCom->Set_Scale(_vector{ 47.9f,50.f,47.9f });
...
void CBeastMap3::Tick(_float fTimeDelta)
{
...
void CBeastMap3::LateTick(_float fTimeDelta)
{
...
HRESULT CBeastMap3::Ready_Components()
{
	/* For.Com_Transform */
...
CBeastMap3 * CBeastMap3::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CBeastMap3*		pInstance = new CBeastMap3(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CBeastMap3"));
...
CEffect * CBeastMap3::Clone(void * pArg)
{
	CBeastMap3*		pInstance = new CBeastMap3(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CBeastMap3"));
		Safe_Release(pInstance);
	}
