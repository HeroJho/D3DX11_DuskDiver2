...
CBear3::CBear3(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CCreture(pDevice, pContext)
{
}

CBear3::CBear3(const CBear3 & rhs)
	: CCreture(rhs)
{
}

HRESULT CBear3::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBear3::Initialize(void * pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(-188.6f, 38.7f, 123.665f, 1.f));
...
void CBear3::Tick(_float fTimeDelta)
{
	...
	switch (m_BearState)
	{
	case Client::CBear3::STATE_UP:
		Pattern_Up(fTimeDelta);
		break;
	case Client::CBear3::STATE_DOWN:
		Pattern_Down(fTimeDelta);
		break;
	case Client::CBear3::STATE_HIDE:
		Pattern_Hide(fTimeDelta);
		break;
	case Client::CBear3::STATE_HIT:
		Pattern_Hit(fTimeDelta);
		break;
	case Client::CBear3::STATE_END:
		break;
	default:
		break;
	}
...
void CBear3::LateTick(_float fTimeDelta)
{
...
void CBear3::OnCollision(CCollider::OTHERTOMECOLDESC Desc)
{
...
void CBear3::On_Hit(CCreture * pOther)
{
	if (m_bHurt)
	{
		m_bHurt = false;
...
HRESULT CBear3::Ready_Components()
{
...
CBear3 * CBear3::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CBear3*		pInstance = new CBear3(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CBear3"));
...
CCreture * CBear3::Clone(void * pArg)
{
	CBear3*		pInstance = new CBear3(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CBear3"));
		Safe_Release(pInstance);
	}
...
void CBear3::Pattern_Up(_float fTimeDelta)
{
	if (m_bAttackStart)
	{
...
void CBear3::Pattern_Down(_float fTimeDelta)
{
...
void CBear3::Pattern_Hit(_float fTimeDelta)
{
...
void CBear3::Pattern_Hide(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
