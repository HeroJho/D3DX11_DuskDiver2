...
CPunchScore3::CPunchScore3(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CPunchScore3::CPunchScore3(const CPunchScore3 & rhs)
	: CGameObject(rhs)
{
}

HRESULT CPunchScore3::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CPunchScore3::Initialize(void * pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(136.5f, -13.f, 82.278f, 1.f));

	return S_OK;
}

void CPunchScore3::Tick(_float fTimeDelta)
{
...
void CPunchScore3::LateTick(_float fTimeDelta)
{
...
HRESULT CPunchScore3::Render()
{
	...
	if (m_bLight)
	{
		...
		{
			_int i = (_int)(TM->Get_FunchCuma()->Get_Score()) % 1000 - (_int)(TM->Get_FunchCuma()->Get_Score()) % 100 - (_int)(TM->Get_FunchCuma()->Get_Score()) % 10;
...
void CPunchScore3::Render_Debug()
{
...
HRESULT CPunchScore3::Ready_Components()
{
	/* For.Com_Transform */
...
CPunchScore3 * CPunchScore3::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CPunchScore3*		pInstance = new CPunchScore3(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CPunchScore3"));
...
CGameObject * CPunchScore3::Clone(void * pArg)
{
	CPunchScore3*		pInstance = new CPunchScore3(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CPunchScore3"));
		Safe_Release(pInstance);
	}
