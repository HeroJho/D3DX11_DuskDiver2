...
CHolo_4::CHolo_4(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CHolo_4::CHolo_4(const CHolo_4 & rhs)
	: CGameObject(rhs)
{
}

HRESULT CHolo_4::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CHolo_4::Initialize(void * pArg)
{
...
void CHolo_4::Tick(_float fTimeDelta)
{
	m_fTimeDelta += fTimeDelta;
	m_fTimeDeltaHoloAcc += fTimeDelta;
	if (0.5f < m_fTimeDeltaHoloAcc)
	{
		m_iHoloIndex = GI->Get_Random(0, 3);
		m_fTimeDeltaHoloAcc = 0.f;
	}
...
void CHolo_4::LateTick(_float fTimeDelta)
{
...
HRESULT CHolo_4::Render()
{
	...
	if (FAILED(m_pTextureCom->Set_SRV(m_pShaderCom, "g_NoiseTexture", m_iHoloIndex)))
		return E_FAIL;
...
void CHolo_4::OnCollision(CCollider::OTHERTOMECOLDESC Desc)
{

}


HRESULT CHolo_4::Ready_Components()
{
	/* For.Com_Transform */
	...
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Texture_Holo_2"), TEXT("Com_Texture"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
...
CHolo_4 * CHolo_4::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CHolo_4*		pInstance = new CHolo_4(pDevice, pContext);
...
CGameObject * CHolo_4::Clone(void * pArg)
{
	CHolo_4*		pInstance = new CHolo_4(*this);
