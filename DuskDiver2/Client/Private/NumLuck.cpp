#include "stdafx.h"
#include "..\Public\NumLuck.h"
#include "GameInstance.h"
#include "UI_Manager.h"


CNumLuck::CNumLuck(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CUI(pDevice, pContext)
{

}

CNumLuck::CNumLuck(const CNumLuck & rhs)
	: CUI(rhs)
{
}

HRESULT CNumLuck::Initialize_Prototype()
{
...
HRESULT CNumLuck::Initialize(void * pArg)
{
...
void CNumLuck::Tick(_float fTimeDelta)
{
...
void CNumLuck::LateTick(_float fTimeDelta)
{
...
HRESULT CNumLuck::Render()
{
	...
	for (_uint i = 0; i < m_sNumber.size(); ++i)
	{
		...
		if (UM->Get_StatusNum() == 7 && UM->Get_Reserver() > 0)
		{
...
void CNumLuck::Set_Number(_uint iNum)
{
	m_iNumber = iNum;
...
HRESULT CNumLuck::Ready_Components()
{
...
CNumLuck * CNumLuck::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CNumLuck*		pInstance = new CNumLuck(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CNumLuck"));
...
CGameObject * CNumLuck::Clone(void * pArg)
{
	CNumLuck*		pInstance = new CNumLuck(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CNumLuck"));
		Safe_Release(pInstance);
	}
