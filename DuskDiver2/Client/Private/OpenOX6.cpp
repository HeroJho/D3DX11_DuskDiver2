...
COpenOX6::COpenOX6(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

COpenOX6::COpenOX6(const COpenOX6 & rhs)
	: CGameObject(rhs)
{
}

HRESULT COpenOX6::Initialize_Prototype()
{
	return S_OK;
}

HRESULT COpenOX6::Initialize(void * pArg)
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(-50.8f, 17.5f, 108.1f, 1.f));

	return S_OK;
}

void COpenOX6::Tick(_float fTimeDelta)
{
	if (UM->Get_Trigger6())
	{
		if (!m_bTriggerCheck)
	...
	m_pTransformCom->Rotation(XMVectorSet( 0,1.f,0,0), 30.3f);

}

void COpenOX6::LateTick(_float fTimeDelta)
{
...
void COpenOX6::Render_Debug()
{
}

HRESULT COpenOX6::Ready_Components()
{
	/* For.Com_Transform */
...
COpenOX6 * COpenOX6::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	COpenOX6*		pInstance = new COpenOX6(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : COpenOX6"));
...
CGameObject * COpenOX6::Clone(void * pArg)
{
	COpenOX6*		pInstance = new COpenOX6(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : COpenOX6"));
		Safe_Release(pInstance);
	}
