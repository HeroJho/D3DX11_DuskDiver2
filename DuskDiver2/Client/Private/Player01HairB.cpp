#include "stdafx.h"
#include "..\Public\Player01HairB.h"
...
CPlayer01HairB::CPlayer01HairB(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CParts(pDevice, pContext)
{
}

CPlayer01HairB::CPlayer01HairB(const CPlayer01HairB & rhs)
	: CParts(rhs)
{
}

HRESULT CPlayer01HairB::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CPlayer01HairB::Initialize(void * pArg)
{
...
void CPlayer01HairB::Tick(_float fTimeDelta)
{
	
}

void CPlayer01HairB::LateTick(_float fTimeDelta)
{
...
void CPlayer01HairB::Render_Debug()
{
...
void CPlayer01HairB::OnCollision(CCollider::OTHERTOMECOLDESC Desc)
{


}

void CPlayer01HairB::Render_Vtx()
{
...
PxRigidDynamic * CPlayer01HairB::Find_Rigid(const char * pName, CHierarchyNode** Out_pHeadBone)
{
...
void CPlayer01HairB::SetInitPos()
{
	Set_Wake();
...
void CPlayer01HairB::Set_Sleep()
{
	...
	m_pHeadRigid->setActorFlag(PxActorFlag::eDISABLE_SIMULATION, true);
}

void CPlayer01HairB::Set_Wake()
{
	_uint iSize = (_uint)m_pRigid.size();
	for (_uint i = 0; i < iSize; ++i)
	{
	...
	m_pHeadRigid->setActorFlag(PxActorFlag::eDISABLE_SIMULATION, false);

}

_float4x4 CPlayer01HairB::Get_ConbineMatrix()
{
	_float4x4 Worldmatrix;
...
HRESULT CPlayer01HairB::Ready_Components()
{
	/* For.Com_Shader */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, L"Player01HairB", TEXT("Player01HairB"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CPlayer01HairB::Ready_JointHair()
{
...
CPlayer01HairB * CPlayer01HairB::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CPlayer01HairB*		pInstance = new CPlayer01HairB(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CPlayer01HairB"));
...
CGameObject * CPlayer01HairB::Clone(void * pArg)
{
	CPlayer01HairB*		pInstance = new CPlayer01HairB(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CPlayer01HairB"));
		Safe_Release(pInstance);
	}
...
void CPlayer01HairB::Render_HairCol()
{
