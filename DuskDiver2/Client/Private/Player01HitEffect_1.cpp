#include "stdafx.h"
#include "..\Public\Player01HitEffect_1.h"
#include "GameInstance.h"
#include "Time_Manager.h"

CPlayer01HitEffect_1::CPlayer01HitEffect_1(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CPlayer01HitEffect_1::CPlayer01HitEffect_1(const CPlayer01HitEffect_1 & rhs)
	: CGameObject(rhs)
{
}

HRESULT CPlayer01HitEffect_1::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CPlayer01HitEffect_1::Initialize(void * pArg)
{
	m_Info = (*(HITEFFECT1INFO**)pArg);

	if (FAILED(Ready_Components()))
		return E_FAIL;
	
	m_sTag = "Player01HitEffect_1";
	m_pTransformCom->Set_Scale(_vector{ m_Info->vScale.x ,m_Info->vScale.y,m_Info->vScale.z });
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(m_Info->vPosition.x, m_Info->vPosition.y , m_Info->vPosition.z, 1.f));
	m_fAngle = GI->Get_FloatRandom(0.f, 360.f);
	return S_OK;
}

void CPlayer01HitEffect_1::Tick(_float fTimeDelta)
{
...
void CPlayer01HitEffect_1::LateTick(_float fTimeDelta)
{
	Compute_CamZ(m_pTransformCom->Get_State(CTransform::STATE_POSITION));
	m_pRendererCom->Add_RenderGroup(CRenderer::RENDER_ALPHABLEND, this);
}

HRESULT CPlayer01HitEffect_1::Render()
{
...
void CPlayer01HitEffect_1::OnCollision(CCollider::OTHERTOMECOLDESC Desc)
{


}

void CPlayer01HitEffect_1::Render_Debug()
{

}


HRESULT CPlayer01HitEffect_1::Ready_Components()
{
	/* For.Com_Shader */
...
CPlayer01HitEffect_1 * CPlayer01HitEffect_1::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CPlayer01HitEffect_1*		pInstance = new CPlayer01HitEffect_1(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CPlayer01HitEffect_1"));
...
CGameObject * CPlayer01HitEffect_1::Clone(void * pArg)
{
	CPlayer01HitEffect_1*		pInstance = new CPlayer01HitEffect_1(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CPlayer01HitEffect_1"));
		Safe_Release(pInstance);
	}
...
void CPlayer01HitEffect_1::Free()
{
	__super::Free();
