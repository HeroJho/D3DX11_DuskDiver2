#include "stdafx.h"
#include "..\Public\NumLevel.h"
#include "GameInstance.h"
#include "UI_Manager.h"


CNumLevel::CNumLevel(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CUI(pDevice, pContext)
{

}

CNumLevel::CNumLevel(const CNumLevel & rhs)
	: CUI(rhs)
{

}

HRESULT CNumLevel::Initialize_Prototype()
{
...
HRESULT CNumLevel::Initialize(void * pArg)
{
...
void CNumLevel::Tick(_float fTimeDelta)
{
...
void CNumLevel::LateTick(_float fTimeDelta)
{
	...
	for (_uint i = 0; i < m_sNumber.size(); ++i)
	{
		...
		if (UM->Get_ShopState())
		{
			if (UM->Get_ShopSelectBar())
			{
				...
				{
					if ((UM->Get_ShopSelect() == 1 && UM->Get_Page1Select() != 1))
					{
...
void CNumLevel::Set_Number(_uint iNum)
{
	m_iNumber = iNum;
...
HRESULT CNumLevel::Ready_Components()
{
...
CNumLevel * CNumLevel::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CNumLevel*		pInstance = new CNumLevel(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CNumLevel"));
...
CGameObject * CNumLevel::Clone(void * pArg)
{
	CNumLevel*		pInstance = new CNumLevel(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CNumLevel"));
		Safe_Release(pInstance);
	}
